# app/services/ai/extract_proposed_actions.rb
#
...
#
# Usage:
#   Ai::ExtractProposedActions.new(user_message: user_message, context: "...").call!
#
module Ai
  class ExtractProposedActions
    ...
    def request_actions_json(instruction:, assistant_text:, context:)
      system =
        if Ai::ActionCatalog.respond_to?(:extraction_system_prompt)
          Ai::ActionCatalog.extraction_system_prompt
        else
          <<~SYSTEM
      ...
      allowed_types =
        if Ai::ActionCatalog.respond_to?(:allowed_types)
          Ai::ActionCatalog.allowed_types
        else
          Ai::ActionCatalog.types
        end
    ...
    def persist_actions!(ai_message:, extracted_actions:)
      ...
      AiMessage.transaction do
        ...
        actions.each do |a|
          ...
          payload = Ai::ActionCatalog.normalize_payload(type, payload) if Ai::ActionCatalog.respond_to?(:normalize_payload)
          Ai::ActionCatalog.validate!(type, payload)
